...
const scallopSuiConfig: any = JSON.parse(`${import.meta.env.VITE_SCALLOP_SUI_CONFIG}`);
  ...
  if (poolAddressConfigMap.size > 0) {
    ...
    for (let poolConfig of poolAddressConfigMap.values()) {
      ...
      if (poolObjectResp.data?.content) {
        ...
        poolObject.stakeCoinName = poolTypeCommonTypeMap.get(poolObject.poolType).nativeCoinName;
        poolObject.rewardCoinName = poolTypeCommonTypeMap.get(poolObject.poolType).rewardCoinName;
        poolObject.stakeCoinUsdRate = (await getUsdRateByPoolType(poolObject.poolType)).stakeCoinUsdRate;
        poolObject.currentRound = poolData.fields.current_round;

        let currentTimeInfo: any = await getRoundExpireTimeInfo(poolObject.poolId, [poolObject.currentRound]);
        poolObject.currentRoundExpireTime = currentTimeInfo.roundExpireTimeMap.get(poolObject.currentRound);

        poolObject.validatorStatus = {};

        let stakeDecimal = poolTypeCommonTypeMap.get(poolObject.poolType)?.nativeDecimal;
        let rewardDecimal = poolTypeCommonTypeMap.get(poolObject.poolType)?.rewardDecimal;

        // 質押數量
        let statistics: any = new Object();
        if (poolData.fields.statistics) {
          statistics.totalDeposit = poolData.fields.statistics.fields.total_amount / stakeDecimal;
          statistics.totalAttendance = poolData.fields.statistics.fields.user_set.fields.contents.length;
        }
        ...
        timeInfo.rewardDuration = poolData.fields.time_info.fields.reward_duration / 1000;
        timeInfo.startTime = poolData.fields.time_info.fields.start_time;
        timeInfo.expireDuration = poolData.fields.time_info.fields.expire_duration / 1000;
        timeInfo.lockStakeDuration = poolData.fields.time_info.fields.lock_stake_duration / 1000;
        ...
        if (poolConfig.shareSupply !== "") {
          poolObject.shareSupplyInfo = await getShareSupply(poolConfig.shareSupply, stakeDecimal);
        }
...
export async function getPoolRewardInfo(poolType: string) {
  ...
  if (oldTime === "") {
    ...
    // 如果相差時間大於 0，則要把 新的數據變成舊的數據
    oldRewardAmount = newRewardAmount * diffDayTime;
    oldTime = newTime;
  } else {
    diffDayTime = 1;
  }
...
export async function getRoundExpireTimeInfo(poolId: string, roundArray: any[]) {
...
export async function getUserBalanceInfo(address: any, poolType: any) {
  ...
  for (let poolConfig of poolAddressConfigMap.values()) {
    ...
    let stakeCoinType = poolTypeCommonTypeMap.get(poolConfig.poolType).nativeCoinType;
    let stakeCoinName = poolTypeCommonTypeMap.get(poolConfig.poolType).nativeCoinName;
    let stakeCoinDecimal = poolTypeCommonTypeMap.get(poolConfig.poolType).nativeDecimal;
    ...
    userBalanceMap.set(poolConfig.poolType, {
      ...
      totalBalance: Number(walletBalance.totalBalance) / Number(stakeCoinDecimal)
    });
  ...
  if (address) {
    ...
    if (objectResponse.data) {
      ...
      for (let resp of objectResponse.data) {
        ...
        userStakeTotalAmount = Number(userStakeTotalAmount) + Number(realAmount);
...
export async function packStakeTxb(
  ...
  switch (poolType) {
    ...
    case PoolTypeEnum.SCALLOP_PROTOCOL_SUI:
      ...
      typeArgs = [
        SCALLOP_PROTOCOL_SUI_POOL_TYPE,
        SUI_COIN_TYPE
      ]
