module pooltogether::drand_lib {
    ...
    public(package) fun random_index_range(round: u64, drand_sig: vector<u8>, range: u64): u64{
        verify_drand_signature(drand_sig, round);
        let digest = derive_randomness(drand_sig);
        let user_index = safe_selection(range, &digest);
        user_index
    } 
}